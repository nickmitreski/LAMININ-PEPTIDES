 ...
 *    (with PAYMENT_LINK_EMBED + LAMIN_PUBLIC_SITE_URL, SMS defaults to parent link: /pay?pid=&ref= → CoreForge embed)
 *
...
Deno.serve(async (req) => {
  ...
  /** SMS/email link: `parent` = storefront `/pay?pid=&ref=` → iframe CoreForge (metadata from create-payment-link). `direct` = raw CoreForge payment_url. */
  const smsModeRaw = (Deno.env.get('COREFORGE_SMS_PAYMENT_LINK_MODE') ?? '').trim().toLowerCase();
  const smsLinkMode: 'parent' | 'direct' =
    smsModeRaw === 'parent' || smsModeRaw === 'direct'
      ? smsModeRaw
      : coreforgeEmbedCheckout && laminPublic.length > 0
        ? 'parent'
        : 'direct';
