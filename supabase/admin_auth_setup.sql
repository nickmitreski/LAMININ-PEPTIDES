...
-- to match the account you use on /admin/login (example below uses the public site domain).
--
...
WHERE lower(email) = lower('info@lamininpeplab.com.au');

-- Verify:
-- SELECT id, email, raw_app_meta_data FROM auth.users WHERE lower(email) = lower('info@lamininpeplab.com.au');
